...
treatvars <- c(
  "shf_aceisub", "shf_arbsub", "shf_bblsub",
  "shf_rasdosemax", "shf_bbldosemax", "shf_mradosemax",
...
tabout$Variables <- ifelse(stri_extract_first_words(tabout$Variablesforfoot) == "shf_mradosemax",
  paste0(tabout$Variables, footnote_marker_symbol(1)), tabout$Variables
)

tabout$Variables <- ifelse(stri_extract_first_words(tabout$Variablesforfoot) == "supremumdose_sens_cat",
...
footnote(
  ...
  symbol = c(
    ...
    "Includes MRA dose (data only from 2014)."
  )
)
```
